...
@configdataclass
class PinDepsConfig:
    logs: pathlib.Path = field("Path to extracted logs")
    update: bool = field(
...
class PinDepsPlugin:
    ...
    ci_installed: List[str]
    requirements_txt_path: pathlib.Path = None
    requirements_txt_contents: str = None
    requirements_txt_pinned: str = None
...
class PinDeps(CMD):
    ...
    def parse_github_actions_log_file(cls, contents):
        ...
        path = list(filter(lambda i: "PLUGIN=" in i, lines))[0].split("=")[-1]

        python_version = tuple(
            map(
                int,
                list(filter(lambda i: "PYTHON=" in i, lines))[0]
                .split("=")[-1]
                .replace("python", "")
                .split("."),
            )
        ...
        return PinDepsPlugin(
            path=path, python_version=python_version, ci_installed=ci_installed
        )

    async def pin_deps(self, logs_dir):
        plugins = {}

        for filepath in logs_dir.rglob("**/*"):
            if not (
                filepath.parent.name.startswith("test")
                and filepath.name.endswith("Test.txt")
            ):
                continue
            ...
            plugin = self.parse_github_actions_log_file(contents)

            plugins[(plugin.path, plugin.python_version)] = plugin

        for plugin in plugins.values():
            if plugin.python_version != self.LOWEST_PYTHON_VERSION:
                continue
            other_versions_of_plugin = {
                path: other_version_of_plugin
                for (
                    path,
                    python_version,
                ), other_version_of_plugin in plugins.items()
                if path == plugin.path
                and python_version != plugin.python_version
            }

            self.logger.debug(
                "python%d.%d PLUGIN=%s", *plugin.python_version, plugin.path
            )

            plugin_dir = (
                pathlib.Path(__file__)
                .parents[2]
                .joinpath(*plugin.path.split("/"))
            )

            if not plugin_dir.is_dir():
                raise FileNotFoundError(f"plugin_dir {plugin_dir} not found")

            for requirements_txt_path in plugin_dir.glob("requirements.txt"):
                plugin.requirements_txt_path = requirements_txt_path
                plugin.requirements_txt_contents = (
                    requirements_txt_path.read_text()
                )
                requirements_txt_contents = plugin.requirements_txt_contents.split(
                    "\n"
                )
                # Add all packages in requirements files to set of required
                # packages
                for i, line in enumerate(requirements_txt_contents):
                    # Skip comments
                    ...
                    if not package_name in plugin.ci_installed:
                        raise ValueError(
                            f"Python {plugin.python_version}, plugin {plugin.path}: {package_name!r} not in {plugin.ci_installed}"
                        )
                    # TODO Make sure to pin each version also to the version of
                    # Python by using other_versions_of_plugin
                    requirements_txt_contents[i] = (
                        package_name + "==" + plugin.ci_installed[package_name]
                    )

                    self.logger.debug(
                        f"{line:<40} | {requirements_txt_contents[i]}"
                    )
    ...
    async def run(self):
        async for plugin in self.pin_deps(self.logs):
            if self.update:
