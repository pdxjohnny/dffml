...
del DFFML_OVERLAYS_INSTALLED.operations["apply_overlay_to_dataflow_to_be_executed"]
            ...
            type(
                ...
                (cls,),
                {
                    "CONFIG": replace_config(
                        ...
                        cls.CONFIG,
                        {
                            field.name: {"default_factory": lambda: getattr(system_context, field.name)}
...
@config
class DFFMLInstalledOverlaysConfig:
    inputs: List[Input]
    architecture: OpenArchitecture
    orchestrator: BaseOrchestrator

# Create Class for calling operations within the System Context as methods
DFFMLInstalledOverlays = SystemContext.lambdas_as_defaults_for_subclass(
    archietcture=lambda: OpenArchietcture(
        dataflow=DFFML_OVERLAYS_INSTALLED,
    )
    orchestrator=lambda: MemoryOrchestrator(),
)

# Callee
async def apply_overlays_dffml_installed_overlays(dataflow: DataFlow,
        _dffml_system_context):
    ...
    async with self.subflow(use_this_dataflow) as octx:
        async for ctx, results in octx.run(
            [
                ...
                Input(value=into_this_dataflow, definition=DataFlowBeingOverlayed)
            ] + [
                Input(value=and_to_apply_merged_overlay_to_this_dataflow, definition=DataFlowWeAreApplyingOverlaysToByRunningOverlayDataflowAndPassingAsAnInput)
            ],
        ):
            pass
        # We require via manifest/did method style schema for output
