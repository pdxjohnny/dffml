...
from .base import OperationImplementation, \
                  BaseConfig, \
                  BaseContextHandle, \
...
class MemoryInputNetworkContext(BaseInputNetworkContext):
    ...
    async def add(self, input_set: BaseInputSet):
        ...
        if not handle_string in self.parent.input_notification_set:
            self.parent.input_notification_set[handle_string] = NotificationSet()
        ...
        async with self.parent.ctxhd_lock:
            ...
            if not handle_string in self.parent.ctxhd:
                self.parent.ctxhd[handle_string] = \
            ...
            async for item in input_set.inputs():
                ...
                self.parent.ctxhd[handle_string].definitions\
                        [item.definition].append(item)

    async def ctx(self) -> Tuple[bool, BaseInputSetContext]:
        async with self.parent.ctx_notification_set() as ctx:
            return await ctx.added()

    async def added(self, watch_ctx: BaseInputSetContext) -> \
    ...
    async def definition(self, ctx: BaseInputSetContext, definition: str) \
        ...
        async with self.parent.ctxhd_lock:
            ...
            found = list(filter(lambda check: check.name == definition,
                                self.parent.ctxhd[handle_string].definitions))
            # Raise an error if the definition was not found in given context
            if not found:
                raise DefinitionNotInContext('%s: %s' % \
                                             (handle_string, definition,))
            # If found then return the definition
            return found[0]

    def definitions(self, ctx: BaseInputSetContext) \
            -> BaseDefinitionSetContext:
...
class MemoryRedundancyCheckerContext(BaseRedundancyCheckerContext):
    ...
    async def unique(self,
            operation: Operation,
            parameter_set: BaseParameterSet) -> str:
        '''
    ...
    async def exists(self,
        ...
        if await self.kvctx.get(await self.unique(operation, parameter_set)) \
                != '\x01':
            return False
        return True
...
class MemoryOperationImplementationNetworkContext(BaseOperationImplementationNetworkContext):
        ...
        try:
            ...
            raise KeyError(
                    ...
                    % (str(error), operation.name,
                        ', '.join(operation.outputs.keys()))) from error
        # Add the input set made from the outputs to the input set network
        await ictx.add(MemoryInputSet(
            MemoryInputSetConfig(
                ctx=parameter_set.ctx,
                inputs=inputs
            )
        ))
        return inputs
    ...
    async def dispatch(self,
        ...
        task = asyncio.create_task(self.run_dispatch(
            ictx, lctx, operation, parameter_set))
    ...
    async def operations_parameter_set_pairs(self,
        ...
        async for operation in octx.operations(input_set=new_input_set,
                stage=stage):
            # Generate all pairs of un-run input combinations
            async for parameter_set in ictx.gather_inputs(rctx, operation,
                                                          ctx=ctx):
...
class MemoryOrchestratorContext(BaseOrchestratorContext):
    ...
    async def run_operations_for_ctx(self,
        ...
        return ctx, {operation.name: results \
                     ...
                     self.run_stage(ctx, Stage.OUTPUT)}
