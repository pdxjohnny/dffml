...
class ReposTestCase(AsyncTestCase):
    async def setUp(self):
        ...
        #     new=model_load))
        self._stack.enter_context(patch.object(ModelCMD, "arg_model",
                new=ModelCMD.arg_model.modify(type=model_load)))
        self._stack.enter_context(patch("dffml.feature.feature.Feature.load",
            new=feature_load))
        self._stack.enter_context(patch("dffml.df.base.OperationImplementation.load",
            new=opimp_load,
        ))
        self._stack.enter_context(patch("dffml.df.types.Operation.load",
            new=op_load,
        ))
