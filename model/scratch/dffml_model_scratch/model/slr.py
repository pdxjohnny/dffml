...
class SLRContext(ModelContext):
    ...
    async def train(self, sources: Sources):
        # Single Variable
        feature_data = await sources.feature_data(
            self.features + [self.parent.config.predict]
        )
        self.xData = np.array(list(feature_data.values())[1])
        self.yData = np.array(list(feature_data.values())[2])
        self.regression_line = await self.best_fit_line()
        # Mutli Variable
        print(np.array(list(feature_data.values())[:-1]))
        # self.regression_line = await self.best_fit_line()
