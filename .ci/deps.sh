...
export PATH="${TARGET}/miniconda${python_version}/bin:${PATH}"
...
if [[ "x${PLUGIN}" == "xall" ]] || \
  ...
  if [[ "${has_conda}" != "True" ]]; then
    ...
    conda_download="${TARGET}/conda${python_version}.sh"
    ...
    # Run it
    bash "${conda_download}" -u -b -p "${TARGET}/miniconda${python_version}"
    # Update
  ...
  if [ -f "${TARGET}/miniconda${python_version}/bin/activate" ]; then
...
if [[ "x${PLUGIN}" == "xall" ]] || \
   [[ "x${PLUGIN}" == "x." ]] || \
   [[ "x${PLUGIN}" == "xdocs" ]] || \
   [[ "x${PLUGIN}" == "xmodel/vowpalWabbit" ]]; then
  # Errors must be allowed do to some checking for non-existant tools done by
  # conda
  set +e
  conda install -y -c conda-forge vowpalwabbit
  exit_code="$?"
  echo "conda install -y -c conda-forge vowpalwabbit, exit code: ${exit_code}"
  if [[ "1$exit_code" != "10" ]]; then
    exit "${exit_code}"
  fi
  set -e
fi
...
if ([[ "x${PLUGIN}" == "xall" ]] || \
  ...
  exit_code="$?"
  echo "conda install -y -c intel daal4py, exit code: ${exit_code}"
  if [[ "1$exit_code" != "10" ]]; then
    exit "${exit_code}"
  fi
  set -e
