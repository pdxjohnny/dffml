...
def make_ints(strings: List[str]):
    return list(map(int, strings))


HIGH_REGEX = lambda text: make_ints(
    re.compile(r"high=([0-9]+),").findall(text)
)
LOW_REGEX = lambda text: make_ints(re.compile(r"low=([0-9]+),").findall(text))
