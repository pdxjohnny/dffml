...
@op(
    config_class=StaticConfig,
)
def return_static_config(self) -> Any:
    return self.config.static

def static_config(call_this_to_create_object: Callable[[], Any], *args, **kwargs) -> Callable[[], ]:
    return op(
        *args,
...
@op(
    Example=static_config(BaseDataFlowFacilitatorObject),
)
async def double_context_entry(dffml_plugin: BaseDataFlowFacilitatorObject) -> BaseDataFlowFacilitatorObjectContext:
...
def get_config(kvstore_ctx: BaseKeyValueStoreContext) -> Any:
    return kvstore_ctx.get(self, key: str) -> Union[bytes, None]:
...
async for SystemContext() as sysctx:
    async for sysctx() as ctx:
